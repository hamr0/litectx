...
> **POC verdict (2026-06-13, `poc/compress-middle-poc.mjs`) — reframes this section.** The signature-vs-drop **mechanism** is confirmed and useful: signature-tier preserved a middle answer **6/6** (every position) where **drop** lost all 6, at 24% bytes, zero hallucinations — but **lost-in-the-middle did NOT manifest** (verbatim retrieved the middle needle 8/8 at ~4.6 KB, no positional penalty). So **build signature as a rank/recency-driven intermediate BUDGET TIER** (keep verbatim → down-tier to signature → drop), applied to the units FIT would otherwise drop — **not** a positional "middle-band" special case. The positional premise is untested at scale, not refuted; don't let the design hinge on it. Honest limit: signature preserves the **doc/header**, not the body — a body-level answer is lost by signature too (tracked drop, 0/2). See `poc/RESULTS.md` §4.5 gate #2.
...
1. **Scope isolation — is `session` load-bearing? ✅ CLEARED 2026-06-13 → YES, build it.** litectx recall is **relevance-ranked** (BM25 + activation), not a flat history replay — off-session `episode`s might simply *sink*. POC `poc/scope-session-poc.mjs` (18 episodes / 5 sessions, two regimes): foreign-session episodes appear in top-5 in **7/8 (BM25) · 8/8 (embeddings)** queries and **displace** own-session ones in **7/8 · 6/8**. Relevance isolates **only genuinely-orthogonal topics** (the search-vs-auth-vs-billing control); any shared vocabulary — same task *or* same domain — bleeds. Off-session episodes do **not** reliably sink → the volatile-isolation guarantee is a correctness property ranking can't provide → **build the `owner`+`session` model above.** (`poc/RESULTS.md` §4.5 gate #1.)
2. **Compress middle-tier — does it preserve task success? ◑ PARTIAL 2026-06-13 → build signature as a budget tier, not a positional rule.** POC `poc/compress-middle-poc.mjs` (live model, doc + body needles): signature-tier preserved the middle answer **6/6** where **drop** lost all 6 (24% bytes, 0 hallucinations) — but **lost-in-the-middle did not manifest** (verbatim 8/8, no positional penalty at ~4.6 KB). So down-tier to signature is a confirmed **rank/recency-driven intermediate tier** (recover would-be-dropped units as signatures); the **positional "middle valley" framing is untested** — don't build it as a special case. Honest limit: signature keeps the doc/header, not the body (body-needle 0/2). (`poc/RESULTS.md` §4.5 gate #2; reframes §4.3.)
