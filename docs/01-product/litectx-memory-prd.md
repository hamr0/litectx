...
| compress · select · isolate | — | — | — | `compress` covered by unit tests + exercised in `assemble-bench`; **SELECT POC-killed** (CE-PRD §8.2); `isolate` = the owner/session scope filter, covered by `test/scope` |
