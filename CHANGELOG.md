...
- **Docs reorganised into a flat `docs/product/` layout with a generated whole-corpus index.** The numbered `docs/{01-product,02-engineering,03-usage,plans}` folders were flattened into `docs/product/` (history preserved via `git mv`, inbound links rewritten), and the 2,968-line PRD was split into a core `docs/product/litectx-prd.md` plus six themed reference pages under `docs/wiki/` (public API, recall engine, impact/edges, CE primitives, build order, and the CE-Tree appendix), each carrying line citations back to the source. The original PRD is archived byte-identical under `docs/archive/`. A generated `docs/index.md` now maps the whole corpus. Repo-only — none of these paths ship in the npm `files` whitelist, so there is no runtime or published-artifact change.

### Removed

- **Removed the stale `docs/00-context/` directory.** Its substantive contents (`ce-tree.md`, `ce-flow.md`, `ctx-ifra.md`) were folded into `build-studies.md` and the PRD on 2026-06-13; only a now-redundant `README.md` index for that directory remained. Repo-only.
...
- **Restored the ripgrep install step to the publish workflow.** `impact()` shells out to ripgrep (`rg -w`) as a runtime dependency, and `npm test` runs before publishing — the step was dropped when the workflow was regenerated from the template, so the pre-publish suite would have run without `rg` present. `ci.yml` was unaffected and kept its copy.
