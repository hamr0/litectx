...
> [!NOTE]
> **Status: v0.14.0 — semantic by default** (`npm i litectx`). The POC gate has cleared — graph-aware recall beats plain FTS5/BM25 (PRD §11, `poc/RESULTS.md`) — and the surface is **implemented, tested (215 tests), CI-gated, and E2E-validated from the registry**: **recall** (every hit carries a `chunk` pointer — the matching function/section inside the file), **impact**, and **`get(id)`** (the body behind any pointer — written memory verbatim, files fresh from disk) over one shared graph (TS / JS / Python + Markdown), plus the **write path** (`remember`/`forget` for facts, episodes, and runtime docs) and **two consumption surfaces** — a CLI and a zero-dependency stdio **MCP server** (`litectx-mcp`), both thin adapters over the same public API. An **optional Claude Code integration** ships in `integrations/claude/` — an LSP-free pre-edit `impact()` hook and a SessionStart index-warmer (opt-in; nothing in the library depends on it). The deterministic **BM25 + spreading** core is always there; the **embeddings tier** (slice 6) is now **ON by default on the CLI and MCP** (pass `--no-embeddings` for the BM25-only base, e.g. offline or CI). It's what makes memory recall by *meaning*: written facts/episodes match *paraphrases* that share no words with the stored text (slice 11: cosine nominates, not just re-ranks — paraphrase recall 0.000 → 0.574), and it lifts natural-language code recall by ~+0.2 MRR. It rides on `@huggingface/transformers` (an *optional* dep, auto-installed, runs locally via ONNX — no API, no cloud); if it's unavailable, litectx warns once and falls back to BM25. Cost is a one-time ~23 MB model download + index-time embedding; per-query is negligible (~0.7s first load, ~6ms warm). The raw `LiteCtx` library API stays explicit (`embeddings: true`) so embedders opt in deliberately. **New in v0.4.0**: the **complete access-log tier** — `recentActivity()` ("what was I working on" — the chunks litectx witnessed you edit recently), `promotionCandidates()` (hot agent **episodes** to distil into durable **facts**, an explicit ladder into `reviewCandidates`), and **trust columns** on written-memory hits (`provenance`/`use`/`occurredAt`, surfaced for the agent to weigh, never scored). The first **context-engineering render verbs** also ship — **library/orchestration API, deliberately not MCP** (deciding *when* to compress or page a payload out is the host loop's job, not a model verb): **`stash(id,text)`** (restorable compression — park a large payload out of context, keep the cheap `id`, rehydrate with `get`, drop with **`evict`**), its read-half **`peek(id)`** (a `{bytes,head,tail,truncated}` preview without rehydrating), and **`compress(node,{level})`** (rank-tiered render — `verbatim` / `signature` / `drop`; tree-sitter signature extraction saves **~82% of bytes with the doc kept**, methods included). **New in v0.8.0**: **`evict(id | {olderThan, maxCount})`** — the stash-cleanup verb (by id, age, or count), stash-only by construction so a bulk sweep can never reach durable memory; `forget` is now memory-only. **New in v0.9.0**: the **graph-substrate accessors** — **`getNode(id)`** (describe a node's structure: its symbols as `chunks` + **exact** import-edge counts; kind-agnostic) and **`related(id, {dir, hops})`** (walk the persisted `import` graph — `out`/`in`/`both`, multi-hop, capped at 3). The graph is now directly addressable, not just searchable; see the zero-dep human code-map in `examples/graph-view/`. **New in v0.10.0**: the **memory socket** — litectx now drops in as a host's swappable memory backend. **`liteCtxAsStore(lc)`** adapts a `LiteCtx` to the four-method `{ store, search, get, delete }` shape a runtime expects (e.g. bareagent's `Memory`), so swapping a substring-scan store for litectx is one line and the host gains ranked, graph-aware recall. It's powered by two new primitives: **`recall(q, { body: true })`** inlines each hit's content (verbatim memory / the localized chunk / whole-file fallback) so a consumer skips the follow-up `get()`s, and **`remember(id, text, { meta })`** attaches an opaque caller dict that round-trips verbatim through `get`/`recall` but lives in **no FTS table** — sealed, never tokenized or scored (a term only in `meta` can't make the memory recallable). **New in v0.11.0**: the **`assemble(units, ctx) → { units, dropped, tokens }`** read-path keystone (RT-1) — a pure free function a host loop calls to **budget-fit a transcript** for the next model call. It takes a neutral, grammar-stripped **unit** array (`{ id, role, content, kind?, pinned?, atomic?, tokensApprox? }`) plus a token `budget` and returns the fitted **view**: litectx owns content and relevance, never the provider's grammar. Two flags carry the contract — **`pinned`** units never drop or reorder (the system prompt / current task; the budget is the un-pinned room) and **`atomic`** units (a tool-call + its result) are kept-or-dropped **whole**, so broken grammar is unrepresentable. The fit is **recency-anchored** and never reorders (**cache-stable**, deterministic), and `dropped[]` accounts for every elided unit (no silent loss — restorable by `id` from the host's canonical transcript). It ships **FIT** plus a **COMPRESS** budget tier — when the fit would drop a parseable `code`/`doc` unit it is recovered as its `compress()` **signature** instead (`compressed: true`, full body restorable by `id`); validated on real functions through the verb (signature retrieval 8/8 vs drop 0/8, ~81% bytes saved). SELECT (recall-inject) was POC-killed — fetch your own code via `recall`/`get`/`impact`. `assemble` is **async** (the one await is the tree-sitter render). **New in v0.12.0**: the **Isolate scope model** — two optional `LiteCtxConfig` keys, **`owner`** (the actor that owns durable `fact`s) and **`session`** (the run that owns volatile `episode`s), scope written memory so a long-running / multi-agent host keeps contexts from bleeding. `recall` filters **both** the BM25 and embeddings/KNN paths — an **unset** reader sees everything (single-tenant default, byte-identical to before), a **set** reader sees its own + global (`NULL`) memory only. Kind-aware (`fact` owner-scoped & cross-session; `episode` owner + session, own-run; `code`/`doc` never scoped), stored in a non-FTS sibling table (`mem_scope`, zero-migration). Grounded on real session transcripts (gate #1): without it a run's own episodes are **buried by more-relevant other sessions** (recency is not a ranking term); the filter recovers them exactly. Identity is the host's to resolve and thread; litectx stores + filters, never sniffs. **New in v0.13.0: the write-gate emitter** — `remember()` takes an opt-in `writeGate` hook that emits a gate-able `{ type: "memory.write", kind, provenance, text, id, injectionRisk? }` action and checks it **before the write commits** (a `deny` throws `WriteDeniedError`, nothing persists; `allow`/`ask` proceed). litectx states the **source** (`provenance`) plus an optional guardrails `injectionRisk` shape flag and never makes the content verdict — the gate (bareguard, or any `.check`-shaped object) renders deny/ask. New exports `toWriteAction` / `WriteAudit` / `WriteDeniedError`; default unset = no gate, byte-identical to prior writes. **New in v0.14.0: `summaryWindow(units, ctx)`** — the R-C6 rolling-summary read-path verb (composes `assemble`): under budget pressure it keeps the last-N transcript turns verbatim and rolls everything older into one **restorable** summary (folded turns reported in `dropped` with `reason:"summarized"`, recoverable by `id`), then budget-fits — never overflows, never worse than FIT. litectx owns the policy (trigger/N/splice); the host owns the model (`ctx.summarize`, a `(messages) => Promise<string>` — litectx never calls one; bareagent's loop ships it). POC: at equal budget it retained the dropped-turn answers FIT-drop lost (3/3 vs 0/3 on a live model). Still roadmap (🚧): base-level activation — *and* a trust/stability **tie-breaker** — as a *ranking* signal stays deliberately **out**; POCs found both repo-dependent and prone to burying fresh or better-matching answers, so the edit signal lives in those views and trust lives in those columns, **never** in search scores — plus persisted `call` edges (a precise call-graph). Pre-1.0 — the surface is stable enough to use, but the API may still evolve (e.g. `recall()` is now async).
...
The graph substrate is public API: **`getNode(id)`** describes a node (its symbols as `chunks` + exact import-edge counts; kind-agnostic), and **`related(id, {dir, hops})`** walks its persisted `import` edges. Lower-level access is also exported via `Store` (`symbolDefs`, `nodesForPath`, `allSymbolNames`).
...
**`kind` is first-class — and the write path is live.** Files enter via `index()` → `code` / `doc` (Markdown); knowledge that isn't a file enters via `remember()` → `fact` / `episode` / `doc` (an agent's learned facts, session events, runtime FAQs). Written memory lives in the same store, recalls through the same kind-scoped ranking, carries provenance (`by: "human" | "agent"`), and structurally survives re-indexing. Every recall hit is logged — the audit trail behind `reviewCandidates()` (a human validates well-used agent **facts**) and `promotionCandidates()` (the agent distils well-used **episodes** into facts; litectx flags, never summarizes). `recentActivity()` answers "what was I working on" from witnessed edits. Episodes are an ephemeral scratchpad — a 30-day rolling window, auto-pruned on write. These access-log **read views** ship; base-level activation as a *ranking* signal stays out (POC-found repo-dependent). Other doc formats (pdf/docx/txt via `format`) remain schema-reserved, **no migration**. Alongside memory, **`stash(id, text)`** parks a payload that *isn't* memory — restorable compression (R-C4): drop a large tool result or page out of context, keep the cheap `id`, rehydrate with `get(id)`; it's never indexed, never recalled, never pruned. It's a **library/orchestration verb by design** — *not* a CLI or MCP tool, because parking a payload is a runtime mechanic the host loop performs, not a call a reasoning model makes (the MCP surface stays the model's verbs: recall/remember/impact). Its read-half is **`peek(id)`** (R-I3, handle/lazy-load): a `{ bytes, head, tail, truncated }` **head+tail** preview of a parked blob *without* rehydrating it (the conclusion — exit code, failing frame — lives at the end, so the tail matters). The win is the **bounded result** — only ~head+tail bytes come back, so the payload stays out of your context/token budget — reason over the handle, `get(id)` the full body only if you decide you need it. And **`compress(node, { level })`** (R-C7, rank-tiered render) renders a code symbol at one of three fidelities — `verbatim`, `signature` (the declaration header + its doc, implementation body elided — **~82% smaller** on real code; tree-sitter-extracted, so it keeps `export`/`async`/generics, works on methods, and re-attaches the JSDoc/docstring), or `drop` (a `name …` marker) — so a caller packing a token budget shows the top hits in full and the long tail as signatures. Like `stash`/`peek` it's a **library/orchestration verb** (not CLI/MCP): choosing a render tier is the host loop's job, not a model verb.
