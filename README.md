...
> **Status: v0.15.0 — `npm i litectx`.** Pre-1.0: the surface is stable enough to use and CI-gated (240+ tests), but the API may still evolve (`recall()`/`impact()`/`assemble()` are async). Per-release detail lives in the [CHANGELOG](CHANGELOG.md).
