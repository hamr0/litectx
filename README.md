...
| **Memory** | `remember` · `ingest` · `forget` · `purge` · `recentActivity` · `promotionCandidates` · `reviewCandidates` | Knowledge that isn't a repo file — facts, episodes, notes, and **uploaded files** (`ingest`: md/PDF/DOCX/txt/log/csv → extract → segment → recall by meaning; **any other file** → stored **byte-exact**, found by filename; optional lazy parser tier). Per-upload **`scope`** fences one chat/customer from another (multi-tenant? `strictScope: true` + `ctx.scoped(scope)` make a forgotten scope impossible, not just discouraged); optional **`expiresAt`** retention (`purge` reclaims). Same store, same ranking, carries provenance, survives re-index. Episodes auto-prune. |
